...
export function AdminReceipt({ member }: AdminReceiptProps) {
  ...
  useEffect(() => {
    const getImageUrl = async () => {
      ...
      // Get signed URL from Supabase for private files
      try {
        const { data, error } = await supabase.storage
          .from("documents")
          .createSignedUrl(member.photo_url, 3600) // 1 hour expiry

        if (error) {
          console.warn("Error getting signed URL for admin receipt:", error.message)
          setImageUrl(null)
          return
        }
        ...
        <div className="flex justify-center mb-6">
          <div className="relative">
            ...
            <img
              ...
              alt="صورة المنخرط"
              className="relative object-cover rounded-xl shadow-lg border-4 border-white w-[180px] h-[220px]"
