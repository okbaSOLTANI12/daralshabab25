...
export function exportStatisticsToExcel(members: Member[]) {
  ...
  for (let R = range.s.r; R <= range.e.r; ++R) {
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      // تحديد لون الخلفية حسب نوع الصف
      let bgColor = "FFFFFF" // أبيض للبيانات العادية
      let textColor = "000000" // أسود للنص
      let isBold = false

      // العناوين الرئيسية (الصفوف 0-5)
      if (R <= 5) {
        bgColor = "E6E6FA" // بنفسجي فاتح للعناوين
        isBold = true
      }
      // صفوف المجاميع
      else if (ws[cell_address].v && typeof ws[cell_address].v === 'string' && ws[cell_address].v.includes('مجموع')) {
        bgColor = "E6E6FA" // بنفسجي فاتح لصفوف المجاميع
        isBold = true
      }
