...
export async function generateMemberZip(member: Member) {
  ...
  try {
    ...
    // Download and add actual uploaded documents using signed URLs
    await addDocumentToZipWithSignedUrl(imagesFolder, member.photo_url, "photo")
...
function generateAdminReceiptHtml(member: Member, photoBase64: string): string {
  const getPhotoHtml = () => {
    if (!member.photo_url || member.photo_url.startsWith("placeholder/")) {
      return `<img src="${PLACEHOLDER_PHOTO}" alt="صورة المنخرط" class="photo" onerror="this.style.display='none'">`
    }

    // For HTML generation, we'll use a placeholder since we can't generate signed URLs in static HTML
    // The actual photo will be included as a separate file in the ZIP
    return `<img src="${photoBase64}" alt="صورة المنخرط" class="photo" onerror="this.style.display='none'">`
  }
