...
async function fetchFileAsBase64(signedUrl: string): Promise<string> {
  ...
  if (!response.ok) return PLACEHOLDER_PHOTO
...
export async function generateMemberZip(member: Member) {
  ...
  // --- Fetch and embed the user's photo as base64 ---
  let photoBase64 = getLocalBase64(member.member_id, "photo") || PLACEHOLDER_PHOTO
  if (!photoBase64 && member.photo_url && !member.photo_url.startsWith("placeholder/")) {
    let signedUrl = member.photo_url
    if (!member.photo_url.startsWith("http")) {
      const { data } = await supabase.storage.from("documents").createSignedUrl(member.photo_url, 3600)
      if (data?.signedUrl) signedUrl = data.signedUrl
    }
    if (signedUrl) {
      try {
        photoBase64 = await fetchFileAsBase64(signedUrl)
      } catch (e) {
  ...
  // --- Fetch and add all uploaded documents as base64 or from Supabase ---
  async function addDoc(docType: string, folder: JSZip | null, fileUrl: string | null | undefined, defaultExt: string) {
    if (!folder) return;
    let base64 = getLocalBase64(member.member_id, docType)
    if (base64) {
      folder.file(`${docType}${defaultExt}`, base64.split(",")[1], { base64: true })
      return;
    }
    if (!fileUrl || fileUrl.startsWith("placeholder/")) return;
    ...
    if (signedUrl) {
      try {
        ...
        const blob = await response.blob()
        const ext = getFileExtension(fileUrl, blob.type) || defaultExt
        const b64 = await blobToBase64(blob)
        folder.file(`${docType}${ext}`, b64.split(",")[1], { base64: true })
      } catch {}
    }
  }

  try {
    // Generate admin receipt HTML (photo floats top right, smaller)
    ...
    // Candidate receipt, payment, completion as before
    receiptsFolder?.file("candidate-receipt.html", generateCandidateReceiptHtml(member))
    receiptsFolder?.file("payment-certificate.html", generatePaymentCertificate(member))
    receiptsFolder?.file("completion-certificate.html", generateCompletionCertificate(member))
    // Add all uploaded documents
    await addDoc("photo", imagesFolder, member.photo_url, ".jpg")
    await addDoc("birth-certificate", certificatesFolder, member.birth_certificate_url, ".pdf")
    await addDoc("parental-consent", certificatesFolder, member.parental_consent_url, ".pdf")
    await addDoc("medical-certificate", certificatesFolder, member.medical_certificate_url, ".pdf")
...
function generateAdminReceiptHtml(member: Member, photoBase64: string): string {
  const getPhotoHtml = () => {
    if (!member.photo_url || member.photo_url.startsWith("placeholder/")) {
      return `<img src="${PLACEHOLDER_PHOTO}" alt="صورة المنخرط" style="width:90px;height:110px;object-fit:cover;object-position:center top;float:right;margin:0 0 10px 20px;border-radius:10px;border:2px solid #16a34a;box-shadow:0 2px 6px rgba(0,0,0,0.08);" />`
    }
    return `<img src="${photoBase64}" alt="صورة المنخرط" style="width:90px;height:110px;object-fit:cover;object-position:center top;float:right;margin:0 0 10px 20px;border-radius:10px;border:2px solid #16a34a;box-shadow:0 2px 6px rgba(0,0,0,0.08);" />`
  }
