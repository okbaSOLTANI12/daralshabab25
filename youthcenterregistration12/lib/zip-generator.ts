  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        .photo-side {
          ...
          border: 2px solid #16a34a; box-shadow: 0 2px 6px rgba(0,0,0,0.10);
    ...
    <body>
      ...
      <div class="info-block">
        <img src="${photoBase64}" alt="صورة المنخرط" class="photo-side" onerror="this.style.display='none'">
        <div>
          <div><strong>الرقم التعريفي:</strong> ${member.member_id}</div>
          <div><strong>الاسم الكامل:</strong> ${member.first_name} ${member.last_name}</div>
          <div><strong>تاريخ الميلاد:</strong> ${new Date(member.birth_date).toLocaleDateString("ar-DZ")}</div>
          <div><strong>مكان الميلاد:</strong> ${member.birth_place_commune}, ${member.birth_place_wilaya}</div>
          <div><strong>الجنس:</strong> ${member.gender === "male" ? "ذكر" : "أنثى"}</div>
          <div><strong>رقم الهاتف:</strong> ${member.phone}</div>
          <div><strong>المستوى الدراسي:</strong> ${member.education_level}</div>
          <div><strong>رقم بطاقة الانخراط:</strong> ${member.membership_card_number}</div>
          <div><strong>تاريخ التسجيل:</strong> ${new Date(member.registration_date).toLocaleDateString("ar-DZ")}</div>
          <div><strong>حالة الدفع:</strong> <span style="color: #16a34a;">مدفوع ✓</span></div>
        </div>
      </div>
    </body>
    </html>
