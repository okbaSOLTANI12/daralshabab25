...
export async function generateMemberZip(member: Member) {
  ...
  let photoBase64 = PLACEHOLDER_PHOTO
  if (member.photo_url && !member.photo_url.startsWith("placeholder/")) {
    ...
    if (!member.photo_url.startsWith("http")) {
      const { data, error } = await supabase.storage.from("documents").createSignedUrl(member.photo_url, 3600)
  ...
  // Add the photo as a file in the images folder
  if (photoBase64 && imagesFolder) {
    const base64Data = photoBase64.split(",")[1];
    imagesFolder.file("photo.jpg", base64ToUint8Array(photoBase64));
  }

  // Add certificates as files in the certificates folder
  await addDocumentToZipWithSignedUrl(certificatesFolder, member.birth_certificate_url, "birth-certificate")
  await addDocumentToZipWithSignedUrl(certificatesFolder, member.parental_consent_url, "parental-consent")
  await addDocumentToZipWithSignedUrl(certificatesFolder, member.medical_certificate_url, "medical-certificate")
...
async function addDocumentToZipWithSignedUrl(folder: JSZip | null, fileUrl: string | null | undefined, fileName: string) {
  if (!folder || !fileUrl || fileUrl.startsWith("placeholder/")) {
    console.log(`Skipping ${fileName}: no valid file URL`)
    return
  }
  ...
  try {
    ...
    // If it's not a full URL, get a signed URL from Supabase
    if (!fileUrl.startsWith("http")) {
      ...
      if (error) {
        console.warn(`Error creating signed URL for ${fileName} (${fileUrl}):`, error.message)
        return
      }
      if (!data || !data.signedUrl) {
        console.warn(`No signed URL returned for ${fileName} (${fileUrl})`)
        return
      }
...
// Update admin receipt HTML to move and resize the photo at the top
function generateAdminReceiptHtml(member: Member, photoBase64: string): string {
  const getPhotoHtml = () => {
    if (!member.photo_url || member.photo_url.startsWith("placeholder/")) {
      return `<img src="${PLACEHOLDER_PHOTO}" alt="صورة المنخرط" class="photo-top" onerror="this.style.display='none'">`
    }
    return `<img src="${photoBase64}" alt="صورة المنخرط" class="photo-top" onerror="this.style.display='none'">`
  }
  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        .logo { width: 60px; height: 60px; border-radius: 50%; margin: 0 auto 10px auto; display: block; }
