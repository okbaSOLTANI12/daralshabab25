...
export default function RootLayout({
  ...
  // Simple dark mode toggle (can be improved with context)
  const [dark, setDark] = useState(false)

  return (
    <html lang="ar" dir="rtl" className={dark ? "dark" : ""}>
      <head>
      ...
      <body className={`${inter.className} ${cairo.className} bg-background text-foreground min-h-screen flex`}>
        ...
        <aside className="hidden md:flex flex-col w-64 h-screen bg-gradient-to-b from-emerald-700 to-teal-700 text-white shadow-2xl fixed right-0 top-0 z-30">
          ...
          <div className="px-6 py-4 border-t border-emerald-900 flex items-center justify-between">
            <span className="text-xs">الوضع الليلي</span>
            <button
              className="w-10 h-6 bg-emerald-900 rounded-full flex items-center transition-all focus:outline-none"
              onClick={() => setDark((d) => !d)}
              aria-label="تبديل الوضع الليلي"
            >
              <span
                className={`inline-block w-5 h-5 rounded-full bg-white shadow transform transition-transform ${dark ? "translate-x-4" : "translate-x-0"}`}
              ></span>
            </button>
          </div>
        </aside>
        {/* Main Content Area */}
        <div className="flex-1 min-h-screen md:mr-64 bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 transition-colors duration-300">
          {/* Topbar */}
          <header className="sticky top-0 z-20 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg shadow-md border-b border-emerald-200 dark:border-gray-800 flex items-center justify-between px-6 py-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-bold text-emerald-800 dark:text-emerald-300">نظام تسجيل المنخرطين</span>
            </div>
